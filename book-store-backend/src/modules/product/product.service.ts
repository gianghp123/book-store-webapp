...
export class ProductService implements OnModuleInit {
	...
	onModuleInit() {
		this.retrieverService =
			this.client.getService<HybridBookRetrieverService>(
				'HybridBookRetriever',
			);
	}
	...
	async findAll(
		...
		if (query) {
			subQuery.andWhere('product.title ILIKE :query', {
				query: `%${query}%`,
			});
		}
	...
	async create(
		...
		if (savedProduct.book && savedProduct.book.categories) {
			productDto.categories = savedProduct.book.categories.map(
				(category) => CategoryResponseDto.fromEntity(category),
			);
		}
		return productDto;
	}
